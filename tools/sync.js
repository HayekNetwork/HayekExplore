/*
Name: Hayekchain Blockchain syncer
Version: .0.0.2
...
const hykerUnits = require('../lib/hykerUnits.js');
...
const normalizeTX = async (txData, receipt, blockData) => {
  const tx = {
    ...
    value: hykerUnits.toHyker(new BigNumber(txData.value), 'wei'),
    nonce: txData.nonce,
    r: txData.r,
    s: txData.s,
    v: txData.v,
...
const writeTransactionsToDB = async (config, blockData, flush) => {
  ...
  if (blockData && blockData.transactions.length > 0) {
    for (d in blockData.transactions) {
      ...
      const receipt = await web3.hyk.getTransactionReceipt(txData.hash);
      ...
      if (txData.input && txData.input.length > 2) {
        ...
        if (txData.to === null) {
          ...
          const Token = new web3.hyk.Contract(ERC20ABI, contractAddress);
          contractdb.owner = txData.from;
          contractdb.blockNumber = blockData.number;
          contractdb.creationTransaction = txData.hash;
          try {
            const call = await web3.hyk.call({ to: contractAddress, data: web3.utils.sha3('totalSupply()') });
            if (call === '0x') {
          ...
          contractdb.byteCode = await web3.hyk.getCode(contractAddress);
          if (isTokenContract) {
            contractdb.ERC = 2;
          } else {
            // Normal Contract
  ...
  if (flush && self.blocks > 0 || self.blocks >= config.bulkSize) {
    ...
    if (config.settings.useRichList && accounts.length > 0) {
      asyncL.eachSeries(accounts, (account, eachCallback) => {
        ...
        web3.hyk.getCode(account, (err, code) => {
          if (err) {
          ...
          web3.hyk.getBalance(account, blockNumber, (err, balance) => {
            if (err) {
              console.log(err);
            ...
            data[account].balance = parseFloat(web3.utils.fromWei(balance, 'hyker'));
            eachCallback();
          });
        });
      }, (err) => {
...
const listenBlocks = function (config) {
  const newBlocks = web3.hyk.subscribe('newBlockHeaders', (error, result) => {
    if (!error) {
      return;
    }
  ...
  newBlocks.on('data', (blockHeader) => {
    web3.hyk.getBlock(blockHeader.hash, true, (error, blockData) => {
      if (blockHeader === null) {
...
var syncChain = function (config, nextBlock) {
  if (web3.hyk.net.isListening()) {
    ...
    while (nextBlock >= config.startBlock && count > 0) {
      web3.hyk.getBlock(nextBlock, true, (error, blockData) => {
        if (error) {
...
const prepareSync = async (config, callback) => {
  ...
  oldBlockFind.exec(async (err, docs) => {
    if (err || !docs || docs.length < 1) {
      ...
      if (web3.hyk.net.isListening()) {
        const currentBlock = await web3.hyk.getBlockNumber();
        const latestBlock = config.endBlock || currentBlock || 'latest';
        if (latestBlock === 'latest') {
          web3.hyk.getBlock(latestBlock, true, (error, blockData) => {
            if (error) {
...
const runPatcher = async (config, startBlock, endBlock) => {
  if (!web3 || !web3.hyk.net.isListening()) {
  ...
  if (missingBlocks > 0) {
    ...
    while (count < config.patchBlocks && patchBlock <= endBlock) {
      ...
      web3.hyk.getBlock(patchBlock, true, (error, patchData) => {
        if (error) {
