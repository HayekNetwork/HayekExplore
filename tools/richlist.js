...
// RichList for Ghyk Classic, Ghyk
  ...
  asyncL.waterfall([
    function (callback) {
      ...
      if (updateCallback && (len >= 100 || ended)) {
        ...
        // about ~1000 `hyk_getBalance` json rpc calls are possible in one json-rpc batchjob.
        ...
        asyncL.eachSeries(chunks, (chunk, outerCallback) => {
          ...
          asyncL.eachSeries(chunk, (account, eachCallback) => {
            web3.hyk.getCode(account, (err, code) => {
              ...
              web3.hyk.getBalance(account, (err, balance) => {
                if (err) {
                  return eachCallback(err);
                }
...
async function startSync() {
  const latestBlock = await web3.hyk.getBlockNumber();
