...
const hykjs = {
  signer: require("hykjs-signer"),
  account: require("hykjs-account")};
...
describe("account", function () {
  ...
  it("must match the generated address with HykJS-account", () => {
    F.forall([F.NBytes(32)], pvt => {
      const hykfpAcc = Account.fromPrivate(pvt);
      const hykjsAcc = hykjs.account.privateToAccount(pvt);
      return hykfpAcc.address === hykjsAcc.address;
    }, 128);
  });
    ...
    accounts.forEach(function (correctAccount, i) {
      ...
      correctAccount.transactions.forEach(function (transaction) {
        ...
        if (transaction.oldSignature) {
          ...
          //Signs it, using pre-EIP 155 scheme (using the hykjs-signer lib)
          const oldSignature = hykjs.signer.sign(txObj, correctAccount.privateKey);
