...
var Api = function Api(provider) {
  ...
  var encodeABI = function encodeABI(type, value) {
    if (type === "bytes") {
      ...
      throw "Hyk-lib can't encode ABI type " + type + " yet.";
    }
  };

  var sendTransaction = send("hyk_sendTransaction");
  var sendRawTransaction = send("hyk_sendRawTransaction");
  var getTransactionReceipt = send("hyk_getTransactionReceipt");
  var compileSolidity = send("hyk_compileSolidity");
  var call = send("hyk_call");
  var getBalance = send("hyk_getBalance");
  var accounts = send("hyk_accounts");
  ...
  var addTransactionDefaults = async function addTransactionDefaults(tx) {
    var baseDefaults = [tx.chainId || send("net_version")(), tx.gasPrice || send("hyk_gasPrice")(), tx.nonce || send("hyk_getTransactionCount")(tx.from, "latest"), tx.value || "0x0", tx.data || "0x"];
    ...
    var gas = tx.gas ? tx.gas : Nat.div(Nat.mul((await send("hyk_estimateGas")({
      from: tx.from,
      to: tx.to,
      value: tx.value,
      nonce: tx.nonce,
