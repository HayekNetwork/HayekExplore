(function(f){if(typeof exports==="object"&&typeof module!=="undefined"){module.exports=f()}else if(typeof define==="function"&&define.amd){define([],f)}else{var g;if(typeof window!=="undefined"){g=window}else if(typeof global!=="undefined"){g=global}else if(typeof self!=="undefined"){g=self}else{g=this}g.hykers = f()}})(function(){var define,module,exports;return (function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find module '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r})()({1:[function(require,module,exports){
"use strict";
...
Object.defineProperty(exports, "__esModule", { value: true });
// See: https://github.com/hayekchain/wiki/wiki/Hayekchain-Contract-ABI
var address_1 = require("../utils/address");
var abi_coder_1 = require("../utils/abi-coder");
var bignumber_1 = require("../utils/bignumber");
var bytes_1 = require("../utils/bytes");
...
var HykerscanProvider = /** @class */ (function (_super) {
    __extends(HykerscanProvider, _super);
    function HykerscanProvider(network, apiKey) {
        var _this = _super.call(this, network) || this;
        errors.checkNew(_this, HykerscanProvider);
        var name = 'invalid';
        if (_this.network) {
        ...
        switch (name) {
            case 'homestead':
                baseUrl = 'https://api.hykerscan.io';
                break;
            case 'ropsten':
                baseUrl = 'https://api-ropsten.hykerscan.io';
                break;
            case 'rinkeby':
                baseUrl = 'https://api-rinkeby.hykerscan.io';
                break;
            case 'kovan':
                baseUrl = 'https://api-kovan.hykerscan.io';
                break;
            default:
    ...
    HykerscanProvider.prototype.perform = function (method, params) {
        //if (!params) { params = {}; }
        ...
        switch (method) {
            case 'getBlockNumber':
                url += '/api?module=proxy&action=hyk_blockNumber' + apiKey;
                return web_1.fetchJson(url, null, getJsonResult);
            case 'getGasPrice':
                url += '/api?module=proxy&action=hyk_gasPrice' + apiKey;
                return web_1.fetchJson(url, null, getJsonResult);
            case 'getBalance':
            ...
            case 'getTransactionCount':
                url += '/api?module=proxy&action=hyk_getTransactionCount&address=' + params.address;
                url += '&tag=' + params.blockTag + apiKey;
                return web_1.fetchJson(url, null, getJsonResult);
            case 'getCode':
                url += '/api?module=proxy&action=hyk_getCode&address=' + params.address;
                url += '&tag=' + params.blockTag + apiKey;
                return web_1.fetchJson(url, null, getJsonResult);
            case 'getStorageAt':
                url += '/api?module=proxy&action=hyk_getStorageAt&address=' + params.address;
                url += '&position=' + params.position;
                url += '&tag=' + params.blockTag + apiKey;
                return web_1.fetchJson(url, null, getJsonResult);
            case 'sendTransaction':
                url += '/api?module=proxy&action=hyk_sendRawTransaction&hex=' + params.signedTransaction;
                url += apiKey;
                return web_1.fetchJson(url, null, getJsonResult);
            case 'getBlock':
                if (params.blockTag) {
                    url += '/api?module=proxy&action=hyk_getBlockByNumber&tag=' + params.blockTag;
                    url += '&boolean=false';
                    url += apiKey;
                    return web_1.fetchJson(url, null, getJsonResult);
                }
            ...
            case 'getTransaction':
                url += '/api?module=proxy&action=hyk_getTransactionByHash&txhash=' + params.transactionHash;
                url += apiKey;
                return web_1.fetchJson(url, null, getJsonResult);
            case 'getTransactionReceipt':
                url += '/api?module=proxy&action=hyk_getTransactionReceipt&txhash=' + params.transactionHash;
                url += apiKey;
                return web_1.fetchJson(url, null, getJsonResult);
            case 'call':
                ...
                url += '/api?module=proxy&action=hyk_call' + transaction;
                url += apiKey;
                return web_1.fetchJson(url, null, getJsonResult);
            case 'estimateGas':
                var transaction = getTransactionString(params.transaction);
                ...
                url += '/api?module=proxy&action=hyk_estimateGas&' + transaction;
                url += apiKey;
                return web_1.fetchJson(url, null, getJsonResult);
            case 'getLogs':
            ...
            case 'getHykerPrice':
                ...
                url += '/api?module=stats&action=hykprice';
                url += apiKey;
                return web_1.fetchJson(url, null, getResult).then(function (result) {
                    return parseFloat(result.hykusd);
                });
            default:
                break;
        }
    ...
    HykerscanProvider.prototype.getHistory = function (addressOrName, startBlock, endBlock) {
        var url = this.baseUrl;
        var apiKey = '';
        if (this.apiKey) {
            apiKey += '&apikey=' + this.apiKey;
    ...
    return HykerscanProvider;
}(provider_1.Provider));
exports.HykerscanProvider = HykerscanProvider;
...
var hykerscan_provider_1 = require("./hykerscan-provider");
exports.HykerscanProvider = hykerscan_provider_1.HykerscanProvider;
...
function getDefaultProvider(network) {
    return new fallback_provider_1.FallbackProvider([
        ...
        new hykerscan_provider_1.HykerscanProvider(network),
    ]);
}
exports.getDefaultProvider = getDefaultProvider;

},{"./hykerscan-provider":53,"./fallback-provider":54,"./infura-provider":56,"./ipc-provider":42,"./json-rpc-provider":57,"./provider":58,"./web3-provider":59}],56:[function(require,module,exports){
'use strict';
var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
...
Object.defineProperty(exports, "__esModule", { value: true });
// See: https://github.com/hayekchain/wiki/wiki/JSON-RPC
var provider_1 = require("./provider");
var address_1 = require("../utils/address");
var bytes_1 = require("../utils/bytes");
var networks_1 = require("../utils/networks");
...
var JsonRpcSigner = /** @class */ (function (_super) {
    ...
    JsonRpcSigner.prototype.getAddress = function () {
        ...
        return this.provider.send('hyk_accounts', []).then(function (accounts) {
    ...
    JsonRpcSigner.prototype.sendTransaction = function (transaction) {
        ...
        return properties_1.resolveProperties(tx).then(function (tx) {
            ...
            return _this.provider.send('hyk_sendTransaction', [tx]).then(function (hash) {
                return web_1.poll(function () {
    ...
    JsonRpcSigner.prototype.signMessage = function (message) {
        ...
        return this.getAddress().then(function (address) {
            ...
            return _this.provider.send('hyk_sign', [address.toLowerCase(), bytes_1.hexlify(data)]);
        });
    };
...
var JsonRpcProvider = /** @class */ (function (_super) {
    ...
    JsonRpcProvider.prototype.listAccounts = function () {
        return this.send('hyk_accounts', []).then(function (accounts) {
    ...
    JsonRpcProvider.prototype.perform = function (method, params) {
        switch (method) {
            case 'getBlockNumber':
                return this.send('hyk_blockNumber', []);
            case 'getGasPrice':
                return this.send('hyk_gasPrice', []);
            case 'getBalance':
                return this.send('hyk_getBalance', [getLowerCase(params.address), params.blockTag]);
            case 'getTransactionCount':
                return this.send('hyk_getTransactionCount', [getLowerCase(params.address), params.blockTag]);
            case 'getCode':
                return this.send('hyk_getCode', [getLowerCase(params.address), params.blockTag]);
            case 'getStorageAt':
                return this.send('hyk_getStorageAt', [getLowerCase(params.address), params.position, params.blockTag]);
            case 'sendTransaction':
                return this.send('hyk_sendRawTransaction', [params.signedTransaction]);
            case 'getBlock':
                if (params.blockTag) {
                    return this.send('hyk_getBlockByNumber', [params.blockTag, false]);
                }
                else if (params.blockHash) {
                    return this.send('hyk_getBlockByHash', [params.blockHash, false]);
                }
                return Promise.reject(new Error('invalid block tag or block hash'));
            case 'getTransaction':
                return this.send('hyk_getTransactionByHash', [params.transactionHash]);
            case 'getTransactionReceipt':
                return this.send('hyk_getTransactionReceipt', [params.transactionHash]);
            case 'call':
                return this.send('hyk_call', [JsonRpcProvider.hexlifyTransaction(params.transaction), 'latest']);
            case 'estimateGas':
                return this.send('hyk_estimateGas', [JsonRpcProvider.hexlifyTransaction(params.transaction)]);
            case 'getLogs':
                if (params.filter && params.filter.address != null) {
                    params.filter.address = getLowerCase(params.filter.address);
                }
                return this.send('hyk_getLogs', [params.filter]);
            default:
                break;
        }
    ...
    JsonRpcProvider.prototype._startPending = function () {
        ...
        var self = this;
        var pendingFilter = this.send('hyk_newPendingTransactionFilter', []);
        this._pendingFilter = pendingFilter;
        pendingFilter.then(function (filterId) {
            function poll() {
                self.send('hyk_getFilterChanges', [filterId]).then(function (hashes) {
                    ...
                    if (self._pendingFilter != pendingFilter) {
                        self.send('hyk_uninstallFilter', [filterId]);
                        return;
                    }
...
var Provider = /** @class */ (function (_super) {
    ...
    Provider.prototype.getHykerPrice = function () {
        var _this = this;
        return this.ready.then(function () {
            return _this.perform('getHykerPrice', {}).then(function (result) {
...
var Web3Provider = /** @class */ (function (_super) {
    ...
    Web3Provider.prototype.send = function (method, params) {
        var _this = this;
        // Metamask complains about hyk_sign (and on some versions hangs)
        if (method == 'hyk_sign' && this._web3Provider.isMetaMask) {
...
// http://hayekchain.stackexchange.com/questions/760/how-is-the-address-of-an-hayekchain-contract-computed
...
exports.ConstantWeiPerHyker = bigNumberify('1000000000000000000');
...
function hashMessage(message) {
    var payload = bytes_1.concat([
        utf8_1.toUtf8Bytes('\x19Hayekchain Signed Message:\n'),
...
exports.formatHyker = units_1.formatHyker;
exports.parseHyker = units_1.parseHyker;
...
// NFKC (composed)
var hykerSymbol = '\u039e';
exports.hykerSymbol = hykerSymbol;
var constants = {
    ...
    WeiPerHyker: bignumber_1.ConstantWeiPerHyker
};
exports.constants = constants;
...
function isCrowdsaleWallet(json) {
    ...
    return (data.encseed && data.hykaddr);
}
...
function getJsonWalletAddress(json) {
    if (isCrowdsaleWallet(json)) {
        try {
            return address_1.getAddress(JSON.parse(json).hykaddr);
        }
        catch (error) {
            return null;
        }
...
"use strict";
//See: https://github.com/hayekchain/wiki/wiki/RLP
Object.defineProperty(exports, "__esModule", { value: true });
var bytes_1 = require("./bytes");
...
function formatHyker(wei, options) {
    return formatUnits(wei, 18, options);
}
exports.formatHyker = formatHyker;
function parseHyker(hyker) {
    return parseUnits(hyker, 18);
}
exports.parseHyker = parseHyker;
...
function decryptCrowdsale(json, password) {
    ...
    // Hayekchain Address
    var hykaddr = address_1.getAddress(searchPath(data, 'hykaddr'));
    // Encrypted Seed
...
function decrypt(json, password, progressCallback) {
    ...
    var getSigningKey = function (key, reject) {
        ...
        if (searchPath(data, 'x-hykers/version') === '0.1') {
            var mnemonicCiphertext = looseArrayify(searchPath(data, 'x-hykers/mnemonicCiphertext'));
            var mnemonicIv = looseArrayify(searchPath(data, 'x-hykers/mnemonicCounter'));
            var mnemonicCounter = new aes_js_1.default.Counter(mnemonicIv);
            var mnemonicAesCtr = new aes_js_1.default.ModeOfOperation.ctr(mnemonicKey, mnemonicCounter);
            var path = searchPath(data, 'x-hykers/path') || HDNode.defaultPath;
...
function encrypt(privateKey, password, options, progressCallback) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        //   - 32 bytes   AES key to encrypt mnemonic with (required here to be Hykers Wallet)
        scrypt_js_1.default(passwordBytes, salt, N, r, p, 64, function (error, progress, key) {
            if (error) {
                error.progress = progress;
                reject(error);
                ...
                // See: https://github.com/hayekchain/wiki/wiki/Web3-Secret-Storage-Definition
                var data = {
                ...
                if (entropy) {
                    ...
                    data['x-hykers'] = {
                        client: client,
                        ghykFilename: ('UTC--' + timestamp + '--' + data.address),
...
var words = "";
var wordlist = null;
...
function register(lang, name) {
    ...
    if (exportWordlist) {
        ...
        if (g.hykers && g.hykers.wordlists) {
            if (!g.hykers.wordlists[name]) {
                properties_1.defineReadOnly(g.hykers.wordlists, name, lang);
            }
        }
    }
}
...
Object.defineProperty(exports, "__esModule", { value: true });
var hykers = __importStar(require("./hykers"));
exports.hykers = hykers;
__export(require("./hykers"));
