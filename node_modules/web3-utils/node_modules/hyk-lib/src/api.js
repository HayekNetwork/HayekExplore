const Api = provider => {
  ...
  const sendTransaction = send("hyk_sendTransaction");
  const sendRawTransaction = send("hyk_sendRawTransaction");
  const getTransactionReceipt = send("hyk_getTransactionReceipt");
  const compileSolidity = send("hyk_compileSolidity");
  const call = send("hyk_call");
  const getBalance = send("hyk_getBalance");
  const accounts = send("hyk_accounts");
  ...
  const addTransactionDefaults = tx =>
    ...
    Promise.all([
      ...
      tx.gasPrice || send("hyk_gasPrice")(),
      tx.nonce || send("hyk_getTransactionCount")(tx.from,"latest"),
      tx.value || "0x0",
      tx.data || "0x"])
    // Add them to tx
    ...
    .then(tx => {
      ...
      if (!tx.gas) {
        ...
        return send("hyk_estimateGas")(estimateTx)
          .then(usedGas => {
