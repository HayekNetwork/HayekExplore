...
var Api = function Api(provider) {
  ...
  var addTransactionDefaults = function addTransactionDefaults(tx) {
    return (
      // Get basic defaults
      Promise.all([tx.chainId || send("net_version")(), tx.gasPrice || send("hyk_gasPrice")(), tx.nonce || send("hyk_getTransactionCount")(tx.from, "latest"), tx.value || "0x0", tx.data || "0x"])
      // Add them to tx
      .then(function (_ref) {
        ...
        if (!tx.gas) {
          ...
          return send("hyk_estimateGas")(estimateTx).then(function (usedGas) {
