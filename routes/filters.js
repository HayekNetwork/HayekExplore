...
function filterTX(txs, value) {
  return txs.map(tx => [tx.hash, tx.blockNumber, tx.from, tx.to, hykerUnits.toHyker(new BigNumber(tx.value), 'hyker'), tx.gas, tx.timestamp, tx.creates]);
}
...
function filterTrace(txs, value) {
  return txs.map((tx) => {
    ...
    if (t.type == 'suicide') {
      ...
      if (t.action.balance) t.value = hykerUnits.toHyker(new BigNumber(t.action.balance), 'wei');
      ...
      t.value = hykerUnits.toHyker(new BigNumber(t.action.value), 'wei');
    }
    return t;
  });
}
...
function datatableTX(txs) {
  return txs.map(tx => [tx.hash, tx.blockNumber, tx.from, tx.to,
    hykerUnits.toHyker(new BigNumber(tx.value), 'wei'), tx.gas, tx.timestamp]);
}

function internalTX(txs) {
  return txs.map(tx => [tx.transactionHash, tx.blockNumber, tx.action.from, tx.action.to,
    hykerUnits.toHyker(new BigNumber(tx.action.value), 'wei'), tx.action.gas, tx.timestamp]);
}
