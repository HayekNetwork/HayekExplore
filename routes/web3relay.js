...
if (web3.hyk.net.isListening()) console.log('Web3 connection established');
else throw 'No connection, please specify web3host in conf.json';

async function detectNode() {
  const nodeInfo = await web3.hyk.getNodeInfo();
...
exports.data = async (req, res) => {
  ...
  if ('tx' in req.body) {
    ...
    Transaction.findOne({ hash: txHash }).lean(true).exec(async (err, doc) => {
      if (err || !doc) {
        web3.hyk.getTransaction(txHash, (err, tx) => {
          if (err || !tx) {
            console.error(`TxWeb3 error :${err}`);
            if (!tx) {
              web3.hyk.getBlock(txHash, (err, block) => {
                if (err || !block) {
            ...
            const ttx = tx;
            ttx.value = hykerUnits.toHyker(new BigNumber(tx.value), 'wei');
            //get TxReceipt status & gasUsed
            web3.hyk.getTransactionReceipt(txHash, (err, receipt) => {
              if (err) {
                console.error(err);
                return;
              }
            ...
            const block = web3.hyk.getBlock(tx.blockNumber, (err, block) => {
      ...
      const latestBlock = await web3.hyk.getBlockNumber() + 1;
      ...
      txResponse.gasPriceGwei = hykerUnits.toGwei(new BigNumber(txResponse.gasPrice), 'wei');
      txResponse.gasPriceHyker = hykerUnits.toHyker(new BigNumber(txResponse.gasPrice), 'wei');
      txResponse.txFee = txResponse.gasPriceHyker * txResponse.gasUsed;
    ...
    if (options.indexOf('balance') > -1) {
      try {
        addrData['balance'] = await web3.hyk.getBalance(addr);
        addrData['balance'] = hykerUnits.toHyker(addrData['balance'], 'wei');
      } catch (err) {
    ...
    if (options.indexOf('count') > -1) {
      try {
        addrData['count'] = await web3.hyk.getTransactionCount(addr);
      } catch (err) {
        console.error(`AddrWeb3 error :${err}`);
        addrData = { 'error': true };
      }
    ...
    if (options.indexOf('bytecode') > -1) {
      try {
        addrData['bytecode'] = await web3.hyk.getCode(addr);
    ...
    Block.findOne({ $or: [{ hash: blockNumOrHash }, { number: blockNumOrHash }] },
      ...
      if (err || !doc) {
        web3.hyk.getBlock(blockNumOrHash, (err, block) => {
          if (err || !block) {
            console.error(`BlockWeb3 error :${err}`);
            res.write(JSON.stringify({ 'error': true }));
          } else {
    ...
    */
    ...
    web3.hyk.getBlock(blockNumOrHash, uncleIdx, (err, uncle) => {
      if (err || !uncle) {
        console.error(`UncleWeb3 error :${err}`);
        res.write(JSON.stringify({ 'error': true }));
      } else {
    ...
    if (req.body.action == 'hashrate') {
      web3.hyk.getBlock('latest', (err, latest) => {
        if (err || !latest) {
          ...
          web3.hyk.getBlock(checknum, (err, block) => {
            if (err || !block) {
              console.error(`StatsWeb3 error :${err}`);
