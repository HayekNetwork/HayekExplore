...
module.exports = function (req, res) {
  ...
  else if (req.body.action == 'miners') { getMinerStats(req, res); } else if (req.body.action == 'hashrate') { ghykashrate(res); } else if (req.body.action == 'hashrates') getHashrates(req, res);
  ...
  async.map(options, (opt, callback) => {
    ...
    if (err) {
      ...
      if (results.length < 2) res.status(500).send();
        ...
        const hykHashrate = parseInt(results[h].instantHashrate);
        const etcDiff = results[c].difficulty.toFixed(2);
        const hykDiff = results[h].difficulty.toFixed(2);
        const etcHykHash = parseInt(100 * etcHashrate / hykHashrate);
        const etcHykDiff = parseInt(100 * etcDiff / hykDiff);
        res.write(JSON.stringify({
          'etcHashrate': etcHashrate,
          'hykHashrate': hykHashrate,
          'etcDiff': etcDiff,
          'hykDiff': hykDiff,
          'etcHykHash': etcHykHash,
          'etcHykDiff': etcHykDiff,
        }));
        res.end();
      }
    }
